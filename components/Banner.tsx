	...
	return (
		<section className="flex flex-col space-y-2 py-16 md:space-y-4 lg:h-[65vh] lg:justify-end lg:pb-12">
			<div className="absolute top-0 left-0 z-[1] h-[95vh] w-[100%]">
				<Image src={`${baseURL}original/${Movie?.backdrop_path || Movie?.poster_path}`} alt={`${Movie?.title}`} fill quality={75} className="object-cover" onLoadingComplete={() => ref.current.remove()} />
				{/* image loading bar */}
				<div
			...
			<div className="flex space-x-3 z-10 relative">
				...
				>
					<FaInfoCircle className="w-5 md:w-8" /> Info
				</button>
			</div>
		</section>
	);
